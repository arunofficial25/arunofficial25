<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=260&color=0:0f0f0f,50:1a1a2e,100:16213e&text=Arun&fontAlign=50&fontAlignY=40&fontSize=48&fontColor=ffffff&desc=Data%20Analyst%20•%20Python%20Developer%20•%20Visualization%20Enthusiast&descAlign=50&descAlignY=60" width="100%" />

# Hey, I'm Arun 👋

<img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=500&size=24&pause=1000&color=58A6FF&center=true&vCenter=true&width=800&lines=Data+Analyst+in+Progress;Python+Developer;Data+Visualization+Enthusiast;Building+Projects+and+Learning+Daily;Turning+Data+into+Stories" alt="Typing SVG" />

</div>

# 🚀 About Me

# 🌌 Current Focus

- 📊 Building Data Analytics Projects
- 🐍 Improving Python Development Skills
- 📈 Learning Advanced Data Visualization
- 🧠 Exploring AI-powered workflows
- 📝 Research & Development Projects
- 🚀 Creating modern portfolio experiences

---

# 🛠 Tech Stack

<div align="center">

### Languages

<img src="https://skillicons.dev/icons?i=python,js,html,css,mysql,c" />

### Frameworks & Tools

<img src="https://skillicons.dev/icons?i=react,tailwind,nodejs,git,github,vscode,mysql,firebase" />

### Data Analytics & Visualization

<img src="https://skillicons.dev/icons?i=python,mysql" />

</div>

---

# 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=arunofficial25&theme=tokyonight&hide border=true&background=0D1117" />

</div>

---

# 📈 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=arunofficial25&theme=tokyo-night&hide border=true&bg color=0d1117" width="100%" />

</div>

---

# 🧠 Featured Skills

<div align="center">

| Domain | Skills |
|---|---|
| 📊 Data Analytics | Pandas, NumPy, Data Cleaning |
| 📈 Visualization | Matplotlib, Seaborn, Dashboards |
| 🐍 Python Development | Automation, APIs, Scripting |
| 🗄 Database | SQL, MySQL |
| 🌐 Frontend | React, Tailwind CSS |
| 📝 Research | IEEE Writing, Comparative Analysis |

</div>

---

# 🚀 Featured Projects

## 🎬 IMDb Movie Data Analysis

> My first complete Data Analytics project using Python.

- Data Cleaning
- Exploratory Data Analysis
- Visualization
- Insight Generation

---

## 🌐 Personal Portfolio Website

> Interactive portfolio with animations & modern UI.

---

## 📄 Research Papers

- Progressive Web Apps vs Native Apps – A Comparative Study
- Region-Adaptive AI Framework for Landslide Risk Prediction
- AI Model for Early Prediction of Urban Power Grid Failures
- Generative AI: A Double-Edged Sword for Cybersecurity 
- Python Based Virtual Assistant
- Healthy Lifestyle Hub
---


---

# 🌍 Connect With Me

<div align="center" style="display:flex;gap:12px;justify-content:center;align-items:center;">
    <a href="https://github.com/arunofficial25" target=" blank" rel="noopener noreferrer">
        <img src="https://skillicons.dev/icons?i=github" alt="GitHub" />
    </a>
    <a href="mailto:arunofficial 25@proton.me">
        <img src="https://skillicons.dev/icons?i=gmail" alt="Email" />
    </a>
</div>

---

# 👀 Profile Views

<div align="center">

<img src="https://komarev.com/ghpvc/?username=arunofficial25&label=Profile%20Views&color=0e75b6&style=flat" />

</div>

---

<div align="center">

### ⚡ Building projects, learning daily, and growing step by step.

</div>
